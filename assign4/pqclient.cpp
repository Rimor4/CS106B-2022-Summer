...
/*
 * Retrieves the top k elements with the highest priority from an input stream.
 *
 * @param stream An input stream containing unsorted DataPoint elements.
 * @param k The number of top elements to retrieve.
 * @return A vector containing the top k descending elements with the highest priority.
 */
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    // First, fill in k elements to reduce unnecessary queue operations like size()
    while (pq.size() < k && stream >> pt) {
        pq.enqueue(pt);
    }

    // Each time a new element is added, the element with the smallest priority value
    // in the queue pops up.
    while (stream >> pt) {
        if (pt.priority > pq.peek().priority) {
            pq.dequeue();
            pq.enqueue(pt);
        }
    }
