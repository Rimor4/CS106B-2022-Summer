...
/*
 * Adds a new element into the queue while maintaining the priority order of the queue.
 */
void PQArray::enqueue(DataPoint elem) {
    ...
    // Compare and swap forward one by one unitl a suitable insertion location is found
    // to maintain the decreasing order of priority value.
...
DataPoint PQArray::peek() const {
    if (isEmpty()) error("PQueue is empty!");
...
STUDENT_TEST("PQArray Big-O test") {
    PQArray pq;
