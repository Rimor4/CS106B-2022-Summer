 ...
 */
const string kYourName = "Fengqi Wang";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "SWNWWSESESENSWWWN";
/**
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    auto collectItem = [&](MazeCell* cell) {
        if (needs.contains(cell->contents)) {
            needs.remove(cell->contents);
        }
    };
    ...
    // Verify the path character by character
    for (char move : moves) {
        switch (move) {
            case 'E':
            ...
            case 'N':
                ...
                break;
            default:
                error("Input path contains an invalid character (any character other than 'N', 'S', 'E', or 'W'");
        }
